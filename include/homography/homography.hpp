...
inline Eigen::Matrix3d findHomography(const EigenSTL::vector_pair_Vector2d_Vector2d& paired_points_src_dst) {

  assert(paired_points_src_dst.size() > 4 &&
         "tool::Homography::findHomography: There must be at least 5 pairs of "
         "points given.");
  ...
  if constexpr (methode == SystemSolverMethode::JACOBI_SCD) {
    Eigen::JacobiSVD<Eigen::MatrixXd> svd(PH, Eigen::ComputeThinU | Eigen::ComputeThinV);
    x = svd.solve(b);
  } else if constexpr (methode == SystemSolverMethode::BDCSVD) {
    Eigen::BDCSVD<Eigen::MatrixXd> bdcsvd(PH, Eigen::ComputeThinU | Eigen::ComputeThinV);
    x = bdcsvd.solve(b);
  } else {
    const std::string error =
        "findHomography: Given Template parameter is not supportet.";
 ...
 */
template <SystemSolverMethode methode>
static Eigen::Matrix3d findHomography(const std::array<EigenSTL::pair_Vector2d_Vector2d, 4>& paired_points_src_dst) {
...
static Eigen::Vector2d computeTransformation(const Eigen::Matrix3d& homography,
                                             const Eigen::Vector2d& src) {
  const Eigen::Vector3d dst_homogeneous = homography * src.homogeneous();
